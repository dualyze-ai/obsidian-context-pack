...
export function buildCoverXhtml(params: {
  ...
  const { title, language, noteCount, generatedDate, coverImageHref } = params;
  const isJa = language === 'ja';
  const noteLabel = isJa ? `${noteCount} ノート` : `${noteCount} note${noteCount !== 1 ? 's' : ''}`;
  const coverImg = coverImageHref
    ? `\n    <div class="cover-image-wrap"><img src="${escapeXml(coverImageHref)}" alt="${escapeXml(title)}" class="cover-image"/></div>`
    : '';
  const blockClass = coverImageHref ? 'cover-block cover-block--has-image' : 'cover-block';
  return `<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE html>
...
<body epub:type="cover" xmlns:epub="http://www.idpf.org/2007/ops" class="cover-page">
  <div class="${blockClass}">
    <h1 class="cover-title">${escapeXml(title)}</h1>${coverImg}
