...
- 不明な要件は実装前に確認する`,
  ...
  brief_health_rating:       (r: string) => `**評価：${r}**`,
  brief_rating_excellent:    '優秀',
  brief_rating_good:         '良好',
  brief_rating_developing:   '発展中',
  brief_rating_sparse:       '未整備',
  ...
  brief_insight_hub:         (name: string) => `「${name}」が主要な知識ハブを形成しています。`,
  brief_insight_hub_themed:  (name: string, themes: string) =>
    `「${name}」が主要な知識ハブで、主なテーマは${themes}です。`,
  brief_insight_second:      (name: string, n: number) =>
    `「${name}」が次の発展領域で、${n}件のノートを含みます。`,
  ...
  oq_under_documented:       (n: number) => `${n}件の頻繁に参照される概念のドキュメントが不足しています。`,
