...
Obsidian・Context Pack・NotebookLMなどのシステムへの言及やドキュメントの改善提案は不要です。`,
  ...
  mode_research_prompt:    '比較・根拠明示・情報の網羅性を重視。可能な限り根拠を示し、推測と事実を区別してください。反証や別解釈がある場合は合わせて示してください。',
  mode_development_prompt: 'タスク分解・実装可能な粒度・コード優先。不明な要件は実装を始める前に確認してください。既存仕様を変更する場合は変更理由と影響範囲を明示してください。',
