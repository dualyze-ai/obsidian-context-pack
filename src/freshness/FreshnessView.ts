...
import { type PackRecord, type PackCheckResult, TARGET_LABEL } from './types';
import { checkAllPacks } from './checker';
...
export class FreshnessView extends ItemView {
  ...
  getViewType(): string {
    return FRESHNESS_VIEW_TYPE;
  }

  getDisplayText(): string {
    return 'Project Knowledge Packs';
  }
  ...
  async refresh(): Promise<void> {
    ...
    this.results = await checkAllPacks(
      this.app,
      packs,
      this.plugin.settings.freshnessSettings,
    );
  ...
  private renderPackList(container: HTMLElement, packs: PackRecord[]): void {
    ...
    for (const result of this.results) {
      const pack = packs.find(
        (p) => `${p.source.type}|${p.source.query}|${p.target}` === result.key,
      );
      if (!pack) continue;
  ...
  private renderPackRow(container: HTMLElement, pack: PackRecord, result: PackCheckResult): void {
    ...
    metaLine.setText(
      `投入 ${moment(pack.createdAt).format('MM/DD')} ・ 現在 ${moment().format('MM/DD')}`,
    );
